...
class BaseModel():
    ...
    #     if not self.isTrain or opt.continue_train or opt.finetuning or opt.eval:
    ...
    def setup(self, opt, parser=None):
        ...
        if self.isTrain:
            ...
            if opt.continue_train or opt.finetuning:
                print("LOADING %s"%(self.name))
                self.load_networks(opt.epoch)
            elif opt.eval or opt.test:
                print("LOADING %s"%(self.name))
                self.load_networks(opt.epoch)
                self.test()
        self.print_networks(opt.verbose)
        ...
        for name in self.model_names:
            if isinstance(name, str):
                ...
                if self.opt.finetuning:
                    ...
                    # load_path = self.opt.finetuning_dir + load_filename
