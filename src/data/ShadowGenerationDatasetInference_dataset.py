        ...
        for root,_,fnames in sorted(os.walk(self.dir_shadowimg)):
            ...
            for name in sorted(fname_int,key=int):
                ...
                if fname.endswith('.png'):
                    ...
                    shadowimg = Image.open(X['shadowimg_path']).convert('RGB').resize((self.opt.loadSize, self.opt.loadSize),Image.NEAREST)
                    ...
                    shadow = Image.open(X['fgshadow_path']).convert('L').resize((self.opt.loadSize, self.opt.loadSize),Image.NEAREST)
                    bg_instance = Image.open(X['bginstance_path']).convert('L').resize((self.opt.loadSize, self.opt.loadSize),Image.NEAREST)
                    bg_shadow = Image.open(X['bgshadow_path']).convert('L').resize((self.opt.loadSize, self.opt.loadSize),Image.NEAREST)
