...
from funit_blocks import LinearBlock, Conv2dBlock, ResBlocks, ActFirstResBlock
...
class Self_Attn(nn.Module):
    ...
    def forward(self,x_f, x_b_total):
        ...
        # final_out = torch.cat([outs, x_f],1)
...
class Attention_UNet_MaskBox_Multitask(nn.Module):
    def __init__(self, input_nc, output_nc, ngf=64, norm_layer=nn.BatchNorm2d, use_dropout=False, n_blocks=6, padding_type='reflect'):
        ...
        # self.fc_b = nn.Linear(ngf*28*28, 4)
        # self.roi = RoIAlign((28, 28), spatial_scale=256/256,sampling_ratio=2)
...
class Attention_Twobranch_UNet_Noshare_bbox(nn.Module):
    ...
    def forward(self, input_img, fo_mask, bg_pure_mask, bg_shadow_mask, bg_instance_mask, instance_rects, shadow_rects):
        ...
        # ### regression bbox version 1: wo roi
        # x = self.avgpool(bottleneck_feature)
        # x = x.view(input_img.size()[0],-1)
        # x = self.fc(x)
        # bbox = torch.sigmoid(x)
...
#                           norm_layer(int(ngf * mult / 2)),
#                           nn.ReLU(True)]
#                 model_1_conv += [nn.ReflectionPad2d(1)]
#                 model_1_conv += [nn.Conv2d(int(ngf * mult / 2), 1, kernel_size=3, padding=0)]
#                 model_1_conv += [nn.Tanh()]
#             elif i == 1:
...
#                            norm_layer(int(ngf * mult / 2)),
#                            nn.ReLU(True)]
#                 model_2_conv += [nn.ReflectionPad2d(1)]
#                 model_2_conv += [nn.Conv2d(int(ngf * mult / 2), 1, kernel_size=3, padding=0)]
#                 model_2_conv += [nn.Tanh()]
#             elif i == 2:
...
#                            norm_layer(int(ngf * mult / 2)),
#                            nn.ReLU(True)]
#                 model_3_conv += [nn.ReflectionPad2d(1)]
#                 model_3_conv += [nn.Conv2d(int(ngf * mult / 2),1, kernel_size=3, padding = 0)]
#                 model_3_conv += [nn.Tanh()]
#
#             else:
...
#         self.model_1_conv = nn.Sequential(*model_1_conv)
#         self.model_2_conv = nn.Sequential(*model_2_conv)
#         self.model_3_conv = nn.Sequential(*model_3_conv)
...
#
#
#         up1 = self.model1(bottleneck_feature)
#         uptanh1 = self.model_1_conv(up1)
#         outputs.append(uptanh1)
#
#         up2 = self.model2(up1)
#         uptanh2 = self.model_2_conv(up2)
#         outputs.append(uptanh2)
#
#         up3 = self.model3(up2)
#         uptanh3 = self.model_3_conv(up3)
#         outputs.append(uptanh3)
#
#         up4 = self.model4(up3)
#         # up44 = nn.Conv2d(up4.size()[1], 1, kernel_size=1, stride=1).cuda()(up4)
#         # uptanh4 = F.tanh(up4)
#         # outputs.append(uptanh4)
#
#         output = self.model_final(up4)
...
#         self.model = nn.Sequential(*model)
#         self.model1 = nn.Sequential(*model1)
#         self.ATTnet = Self_Attn(ngf*4, 'relu')
#
#     def forward(self, input_img, fo_mask, bg_pure_mask, bg_shadow_mask, bg_instance_mask):
#
#         bg_shadow_mask = bg_shadow_mask/2 + 0.5
#         bg_instance_mask = bg_instance_mask/2 + 0.5
#         pixels = torch.unique(bg_instance_mask)
#
#         bg_pairs_feature = []
#         print('pairs number', len(pixels))
#         if len(pixels)>5:
#             number = 5
#         else:
#             number = len(pixels)
#         for i in range(number):
#             current_instance = bg_instance_mask.clone()
#             current_shadow = bg_shadow_mask.clone()
#             current_instance[current_instance!=pixels[i]] = 0
#             current_instance[current_instance==pixels[i]] = 1
#             current_shadow[current_shadow!=pixels[i]] = 0
#             current_shadow[current_shadow==pixels[i]] = 1
#             current_instance_shadow_pair = current_instance + current_shadow
#             current_instance_shadow_pair = current_instance_shadow_pair*2 -1
#             current_bg_pair = torch.cat([input_img,current_instance_shadow_pair],1)
#             bg_pairs_feature.append(self.model(current_bg_pair))
#
#         fo_feature = self.model(torch.cat([input_img, fo_mask], 1))
#         bg_pure_feature = self.model(torch.cat([input_img, bg_pure_mask], 1))
#         bg_pairs_feature.append(bg_pure_feature)
#
#
#         bottleneck_feature = self.ATTnet(fo_feature, bg_pairs_feature)
#         output = self.model1(bottleneck_feature)
#         return output
...
# class UnetGenerator_Twobranch(nn.Module):
#     def __init__(self, input_nc, output_nc, num_downs, ngf=64,
#                  norm_layer=nn.BatchNorm2d, use_dropout=False):
#         super(UnetGenerator_Twobranch, self).__init__()
#
#         # construct unet structure
#         unet_block = UnetSkipConnectionBlock(ngf * 8, ngf * 8, input_nc=None, submodule=None, norm_layer=norm_layer, innermost=True)
#         for i in range(num_downs - 5):
#             unet_block = UnetSkipConnectionBlock(ngf * 8, ngf * 8, input_nc=None, submodule=unet_block, norm_layer=norm_layer, use_dropout=use_dropout)
#
#         unet_block1, unet_block2 = [], []
#         unet_block1 = UnetSkipConnectionBlock(ngf * 4, ngf * 8, input_nc=None, submodule=unet_block, norm_layer=norm_layer)
#         unet_block1 = UnetSkipConnectionBlock(ngf * 2, ngf * 4, input_nc=None, submodule=unet_block1, norm_layer=norm_layer)
...
class Adain_Mask_Generator(nn.Module):
    def __init__(self, nf, input_dim_c,input_dim_s, output_dim, nf_mlp,down_class,down_content,n_mlp_blks,n_res_blks,latent_dim):
        super(Adain_Mask_Generator, self).__init__()
        # nf = hp['nf']
        # nf_mlp = hp['nf_mlp']
    ...
    def deconde_from_adain_parameters(self, content, adain_params):
        assign_adain_params(adain_params, self.dec)
        images = self.dec(content)
        return images
...
# import numpy as np
class STN_Network(nn.Module):
    def __init__(self,input_channel,ngf):
        super(STN_Network, self).__init__()
        self.local_net = LocalNetwork(input_channel, ngf)

    def forward(self, bg_object, bg_shadow, fg_object):
        '''
        :param img: (b, c, h, w)
        :return: (b, c, h, w)
        '''
        transform_img = self.local_net(bg_object, bg_shadow, fg_object)
        return transform_img


class LocalNetwork(nn.Module):
    def __init__(self, channel_input, ngf):
        ...
        #     nn.Tanh(),
        # )
        self.conv1 = nn.Conv2d(channel_input, ngf, kernel_size=2)
        self.conv2 = nn.Conv2d(ngf, ngf*2, kernel_size=2)
        self.conv3 = nn.Conv2d(ngf*2, ngf*4, kernel_size=2)
        self.conv4 = nn.Conv2d(ngf*4, ngf*8, kernel_size=2)
        self.conv2_drop = nn.Dropout2d()
        self.fc1 = nn.Linear(ngf*8*2, 128)
    ...
    def forward(self, bg_object, bg_shadow, fg_object):
        ...
        bg_object = F.relu(F.max_pool2d(self.conv1(bg_object), 2))
        bg_object = F.relu(F.max_pool2d(self.conv2(bg_object), 2))
        bg_object = F.relu(F.max_pool2d(self.conv3(bg_object), 2))
        bg_object = F.relu(F.max_pool2d(self.conv4(bg_object), 2))
        bg_object = self.avgpool(bg_object)
        bg_object = bg_object.view(-1,bg_object.size()[1])

        bg_shadow = F.relu(F.max_pool2d(self.conv1(bg_shadow), 2))
        bg_shadow = F.relu(F.max_pool2d(self.conv2(bg_shadow), 2))
        bg_shadow = F.relu(F.max_pool2d(self.conv3(bg_shadow), 2))
        bg_shadow = F.relu(F.max_pool2d(self.conv4(bg_shadow), 2))
        bg_shadow = self.avgpool(bg_shadow)
        bg_shadow = bg_shadow.view(-1,bg_shadow.size()[1])

        fusion_feature = torch.cat([bg_object, bg_shadow],1)
        # print('111', fusion_feature.size())
        x = F.relu(self.fc1(fusion_feature))
        theta = F.relu(self.fc(x))


        batch_size = fg_object.size(0)
        theta = theta.view(batch_size, 2, 3)
        grid = F.affine_grid(theta, torch.Size((batch_size, 1, fg_object.size()[2],fg_object.size()[3] )))
        img_transform = F.grid_sample(fg_object, grid)
